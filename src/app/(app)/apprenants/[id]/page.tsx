...
export default async function PageApprenant({
  ...
  const apprenant = await prisma.learner.findFirst({
    ...
    include: {
      ...
      // Réponses aux questionnaires qualité, lisibles depuis la fiche et non
      // plus seulement depuis l'écran central.
      questionnaires: {
  ...
  return (
    <>
      ...
      {apprenant.questionnaires.length > 0 && (
        <section className="mt-4 rounded-xl border border-bordure bg-surface p-5 shadow-sm">
          <h2 className="mb-3 text-[14.5px] font-bold">
            ...
            <span className="font-normal text-texte-tenu">({apprenant.questionnaires.length})</span>
          </h2>
          <div className="flex flex-col gap-4">
            {apprenant.questionnaires.map((q) => {
              ...
              return (
                <article key={q.id} className="border-t border-bordure-douce pt-3 first:border-t-0 first:pt-0">
                  ...
                  <p className="mb-2 text-[11.5px] text-texte-tenu">
                    ...
                    répondu le {formaterDate(q.reponduAt)}
                  </p>
