...
import { deposerSignatureOrganisme, retirerSignatureOrganisme, type EtatFormulaire } from "@/app/(app)/parametres/actions";
...
export function SignatureOrganisme({ presente }: { presente: boolean }) {
  const [etat, envoyer] = useActionState<EtatFormulaire, FormData>(deposerSignatureOrganisme, {});
  const [apercu, setApercu] = useState<string | null>(null);
  const [detourage, setDetourage] = useState(true);
  ...
  return (
    <section className="max-w-3xl rounded-xl border border-bordure bg-surface p-5 shadow-sm">
      <h2 className="text-[15px] font-bold">Signature de l&apos;organisme</h2>
      <p className="mb-4 mt-1 text-[12.5px] text-texte-doux">
        Apposée au bas des conventions, attestations et certificats générés. Un scan de votre signature ou de votre
        cachet signé convient : le fond blanc est retiré automatiquement.
      </p>
      ...
      <div className="flex flex-wrap items-start gap-5">
        <div className="flex h-28 w-64 shrink-0 items-center justify-center rounded-lg border border-dashed border-bordure bg-[repeating-conic-gradient(var(--color-surface-creuse)_0_25%,transparent_0_50%)] bg-[length:16px_16px] p-2">
          ...
          <img
            src={apercu ?? "/api/organisme/signature"}
            alt="Signature de l'organisme"
            className="max-h-full max-w-full object-contain"
            onError={(e) => {
              (e.currentTarget as HTMLImageElement).style.visibility = "hidden";
            }}
          />
          {!presente && !apercu && <span className="absolute text-[12px] text-texte-tenu">Aucune signature</span>}
        </div>
        ...
        <form action={envoyer} className="flex-1">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <BoutonEnvoyer libelle="Enregistrer la signature" />
            {presente && (
              <button
                type="submit"
                formAction={retirerSignatureOrganisme}
                formNoValidate
